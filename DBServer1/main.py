    ...
    def do_GET(self):
        ...
        if path == '':
            ...
            if len(query) < 1 or not ('key' in query):
                res = { "error": { "code": 400, "message": "No existe la key en la petición" } }
                response(self,400,res)
                return

            f = open('./DBServer1/database.txt','r')
            ...
            res = { "error": { "code": 404, "message": "No encontrado" } }
    ...
    def do_POST(self):
        ...
        if path == '/transaccion/crear':
            ...
            try:
                ...
                res = { "error": { "code": 400, "message": "El formato no es JSON" } }
                response(self, 400, res)
                return
            
            if not os.path.exists('./DBServer1/database.txt'):
                f = open('./DBServer1/database.txt','x')
                f.write('[]')
                f.close()
            ...
            f = open('./DBServer1/database.txt','w')
            json.dump(data,f)
            f.close()
            ...
            try:
                ...
                res = { "error": { "code": 404, "message": "El formato no es un JSON" } }
                response(self, 404, res)
                return
